...
output_folder=path+"%d_m4/"%indexxx
...
haadf=np.load(path_haadf)
#haadf=np.amin(haadf, axis=1)
haadf/=np.mean(haadf)
#haadf=np.repeat(haadf[:,None], 200, axis=1)
...
pypty_params={
      ...
      'beam_current_step':  [1e0, lambda x: (x>=1)],
      'beam_current': haadf.flatten(),
      'save_dict': True,
      'adaptive_steps': True,
