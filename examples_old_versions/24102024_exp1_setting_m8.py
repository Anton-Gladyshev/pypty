...
for indexxx in range(1,16,1):
    for dd in range(2):
        ...
        output_folder=path+"%d_m8/"%indexxx
        step_size_A=15
        path_json=""
        path_h5=path+name+"_4DSTEM.h5"
        path_raw=path+name+"_4DSTEM.npy"
        path_haadf=path+name+"_HAADF.npy"
        ...
        try:
            ...
            tcbf_px_size=(199*step_size_A)/(tcbf_image.shape[0]-1)
        except:
            path_tcbf_full_image=path+name+"_tcBF.npy" ### Not the full one because the full one has weird shape..
            tcbf_image=np.load(path_tcbf_full_image)
            tcbf_px_size=(200*step_size_A)/(tcbf_image.shape[0])
        try:
            path_tcbf_full_json=path+"20241025_Nr_%d_Apo_AuNP_Gr_HUB_0p3msec_5pA_300nm_2p5um_tcBF_full.json"%(indexxx-1)
        ...
        experimental_params={
            ...
            'scan_size': [200,200],
            'scan_step_A': step_size_A,
            'fov_nm': 19.9*step_size_A,
            'acc_voltage': 200, ## kV
            'defocus': 0, ## Angstrom
        ...
        scx, scy=np.meshgrid(np.arange(0, tcbf_image.shape[1],1)*tcbf_px_size, np.arange(0, tcbf_image.shape[0],1)*tcbf_px_size, indexing="xy")
        ...
        ofy, ofx=get_offset(x_range=200, y_range=200,scan_step_A=pypty_params["scan_step_A"], detector_pixel_size_rezA=5.7176870952051917e-05/(12.4/np.sqrt(200*(200+2*511))), patternshape=[384,384], rot_angle_deg=-1*pypty_params["PLRotation_deg"])
        ...
        tcbf_image_phase=iterative_poisson_solver(laplace=laplace,
            px_size=tcbf_px_size,print_flag=3, hpass=1e-2, lpass=0,
        ...
        obj=pypty_params["obj"]
        obj=obj*np.exp(-0.3j+0.6j*np.random.rand(obj.shape[0], obj.shape[1],1,1))
        pypty_params["obj"]=obj

        try:
            pypty_params["probe"]=np.load(output_folder+"cp.npy")
            pypty_params["aberrations"]=None
        except:
            pass
