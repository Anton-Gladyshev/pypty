...
pypty_params={
          'obj_step':   [1e3, lambda x: x%1==0],
          'probe_step':   [1e-10, lambda x:  x>=0],
          'update_batch':  "full", #'full',# lambda x: 'full' if (x%2==0) else 500,
          'randomize': False,
          #'tilts_step':   [1e-8, lambda x: (x>=1)],
          'probe_pos_step': [1e0, lambda x: (x>=10)],
          # 'tilts': np.zeros((200*200, 6)),
         # 'tilt_mode': 2,
          'no_ambiguity': True,
          'obj_update_delta_beta': True,
        #  'sequence_to_reg_grid': 'lambda x: [x//200, x%200]',
          #'postions_poly_grid': 2,
          #'tilts_poly_grid': 5,
          'save_dict': True,
          'adaptive_steps': True,
          ...
          'smooth_rolloff': 0.1,
         # 'apply_gaussian_filter_amplitude': lambda x: 30 if x<=10 else 0,
          'n_parallel_meas': 8,
...
#pypty_params=create_sequence_box(pypty_params, left=50, top=50, width=75, height=75)
