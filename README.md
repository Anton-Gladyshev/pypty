...
| `compute_batch`          | `"auto"`                | `int`      or `str`      | **Batch size for multislice computation.** Increasing this can speed up reconstruction but requires more GPU memory. Ideally one should balance it, but a default value "auto" will compute it based on estimated memory usage. |
