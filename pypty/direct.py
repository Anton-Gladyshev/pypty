...
from pypty import vaa as pyptyvaa
...
def wdd(pypty_params, eps_wiener=1e-3, thresh=None, save=0):
    ...
    ----------
    pypty_params : dict
        Dictionary containing calibrated parameters, including paths and settings
        for data processing.
    eps_wiener : float, optional
        Epsilon parameter for the Wiener filter. Default is 1e-3.
    thresh : float, optional
        If provided, `eps_wiener` is ignored. Denominator values below this threshold
        are set to 1, and the corresponding numerator values are set to 0.
    save : bool, optional
        Whether to save output files (False by default). Ignored if
        `save_preprocessing_files` is specified in `pypty_params`.
    ...
    try:
        ...
        pass
    sys.stdout.write(
    "\n*****************************************************************************\n***************** Running Wigner Distribution  Deconvolution *****************\n*****************************************************************************\n")
    sys.stdout.flush()
