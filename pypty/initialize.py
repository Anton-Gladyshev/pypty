...
def append_exp_params(experimental_params, pypty_params=None):
    ...
    sample_name = experimental_params.get("sample_name", "")
    ...
    slice_thickness= experimental_params.get("slice_thickness", "auto")
    ...
    if scan_step_A is None:
        try:
            scan_step_A=fov_nm*10/(scan_size[0]-1)
        except:
            pass
    ...
    if special_postions_A is None:
        positions, old_pixel_size=get_positions_pixel_size(x_range, y_range, scan_step_A, detector_pixel_size_rezA=rez_pixel_size_A, patternshape=[mean_pattern.shape[0],mean_pattern.shape[1]], rot_angle_deg=-1*PLRotation_deg, flip_x=False,flip_y=False, print_flag=print_flag, transform_axis_matrix=transform_axis_matrix)
    else:
    ...
    highest_freq_xy = rez_pixel_size_A*(old_shape/2)
    conv_angle_estimate = r*rez_pixel_size_A*wavelength if (conv_semiangle_mrad is None) else 1e-3* conv_semiangle_mrad
    highest_freq_z = conv_angle_estimate*highest_freq_xy
    slice_thicness_estimate = 0.5/highest_freq_z
    
    if slice_thickness=="auto":
        slice_thickness = slice_thicness_estimate
        
    if not(slice_thickness is None):
        num_slices=int(np.ceil(total_thickness/slice_thickness))
    
    print("\nBased on pattern geometry, slice thickness should be %.2f Å. You are going to use a slice thickness of %.2f Å and %d %s."%(slice_thicness_estimate, total_thickness/num_slices, num_slices, "slice" if num_slices==1 else "slices"))
    if num_obj_modes is None:
        ...
        print("\nCreating object guess with shape (%d, %d, %d, %d)"%(shapey,shapex, num_slices,num_obj_modes))
   ...
   # if not(masks is None): pypty_params["algorithm"]="lsq_compressed";
    ...
    pypty_params["sample_name"]=sample_name
    pypty_params["transform_axis_matrix"]=transform_axis_matrix
...
def rotate_scan_grid(pypty_params, angle_deg=None, new_pl_rot=None):
    """
    ...
    if new_pl_rot is None:
        new_pl_rot   = old_pl_rot + angle_deg
    else:
        angle_deg= new_pl_rot - old_pl_rot
...
def prepare_restart(output_folder, new_folder=None):
    """
    ...
    new_folder : str, optional
        New output directory for resumed reconstruction. Default is None.

    Returns
    -------
    dict
        Updated `pypty_params` dictionary including loaded state and 'epoch_prev'.
    ...
    with open(loss_path, 'r') as file:
        ...
        for d in data:
            dat.append(d)
    epoch_prev=np.array(dat)[-1, 0]
    params=pyptyutils.load_params(output_folder+"params.pkl")
    ...
    if save_inter_checkpoints==True:
        load_inter=True
        keep_epoch_prev=True

    last_true_inter=-1
    if save_inter_checkpoints:
        last_true_inter=epoch_prev
    elif type(save_inter_checkpoints)==str or isinstance(save_inter_checkpoints, types.LambdaType):
        if isinstance(save_inter_checkpoints, types.LambdaType) or ("lambda" in save_inter_checkpoints):
            range_epochs=np.arange(0, epoch_prev+1,1)
            save_inter_checkpoints_func=pyptyutils.string_to_lambda(save_inter_checkpoints)
            for r in range_epochs:
                if save_inter_checkpoints_func(r):
                    last_true_inter=r
    last_true_check=-1
    if save_checkpoints_every_epoch:
        last_true_check=epoch_prev
    elif type(save_checkpoints_every_epoch)==str or isinstance(save_checkpoints_every_epoch, types.LambdaType):
        if isinstance(save_checkpoints_every_epoch, types.LambdaType) or ("lambda" in save_checkpoints_every_epoch):
            range_epochs=np.arange(0, epoch_prev+1,1)
            save_inter_checkpoints_func=string_to_lambda(save_checkpoints_every_epoch)
            for r in range_epochs:
                if save_inter_checkpoints_func(r):
                    last_true_check=r

    if last_true_check==-1 and last_true_inter==-1:
        raise ValueError("You don't have any saved checkpoints for this reconstruction to restart from!")
    
    
    if last_true_check>last_true_inter:
        epoch_prev=last_true_check
        try:
            bc=np.load(output_folder+"checkpoint_beam_current_epoch_%d.npy"%last_true_check)
        except:
            bc=None
        try:
            o=np.load(output_folder+"checkpoint_obj_epoch_%d.npy"%last_true_check)
        except:
            o=None
        try:
            p=np.load(output_folder+"checkpoint_probe_epoch_%d.npy"%last_true_check)
        except:
            p=None
        try:
            g=np.load(output_folder+"checkpoint_positions_epoch_%d.npy"%last_true_check)
        except:
            g=None
        try:
            t=np.load(output_folder+"checkpoint_tilts_epoch_%d.npy"%last_true_check)
        except:
            t=None
        try:
            st=np.load(output_folder+"checkpoint_stback_epoch_%d.npy"%last_true_check)
        except:
            st=None
        try:
            ab=np.load(output_folder+"checkpoint_aberrations_epcoh_%d.npy"%last_true_check)
        except:
            ab=None
    else:
        epoch_prev=last_true_inter
        try:
            bc=np.load(output_folder+"cb.npy")
        except:
            bc=None
        try:
            o=np.load(output_folder+"co.npy")
        except:
            o=None
        try:
            p=np.load(output_folder+"cp.npy")
        except:
            p=None
        try:
            g=np.load(output_folder+"cg.npy")
        except:
            g=None
        try:
            t=np.load(output_folder+"ct.npy")
        except:
            t=None
        try:
            st=np.load(output_folder+"cs.npy")
        except:
            st=None
        try:
            ab=np.load(output_folder+"ca.npy")
        except:
            ab=None
    params["epoch_prev"]=epoch_prev
    if not(o is None):
        params["obj"]=o
    if not(p is None):
        params["probe"]=p
    if not(bc is None):
        params["beam_current"]=bc
    if not(g is None):
        params["postions"]=g
    if not(t is None):
        params["tilts"]=t
    if not(st is None):
        params["static_background"]=st
    if not(ab is None):
        params["aberrations_array"]=ab
    params["skip_preprocessing"]=True
    if not(new_folder is None):
        params["output_folder"]=new_folder
    return params
