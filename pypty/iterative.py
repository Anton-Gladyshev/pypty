...
from pypty import objective as pyptyobjective
try:
...
def run(pypty_params):
    ...
    Parameters
    ----------
    pypty_params : dict or str
        Reconstruction parameters dictionary or path to a saved parameters file. Must include
        experiment setup and algorithm settings (see `custom_presets.md` and `reference/initialize.md`).
    Notes
    -----
    ...
    """
    global obj, probe, pool, pinned_pool, positions, positions_correction, tilts, tilts_correction, beam_current, static_background, aberrations_array, history_bfgs
    ...
    backend =  params.get('backend', "gpu") ## here backend is a cold storage for gradients and misc arrays, the multislice calculations will be done on GPU whenever it is avalible. You may try to disable cupy if you don't want this behaviour
    ...
    load_one_by_one= params.get('load_one_by_one', True) ## Only is meaningful for .h5 input datasets. If set to False, complemte dataset will be dumped into RAM, if True, patterns will be processed in a one-by-one fashion.
    if xp==np and cp!=np:
        load_one_by_one=True
    smart_memory = params.get('smart_memory', True) ## If True, cache will be cleaned in places, where large operations are executed. Usually helps to prevent memory leaks.
    remove_fft_cache = params.get('remove_fft_cache', False) ## If True, fft cache will be cleaned on the fly after large FFTs are done to keep more space for other operations.
    compute_batch=params.get('compute_batch', "auto") ## how many patterns to process at once on one GPU. Note that if you have more than one GPU, each GPU will deal with this many patterns, so effectively you get even more parralelization. If you are using CPU, it also makes the computation parallel but it becomes less noticable.
    memory_saturation= params.get('memory_saturation', 0.7)
    force_dataset_dtype=params.get('force_dataset_dtype', default_float_cpu) ## more or less self explaining. Does not affect datasets stored in h5 format
    force_pad=params.get('force_pad', False) ## If there is a parameter data_pad, force_pad applies the padding before the update and stores padded data in RAM during the whole reconstruction. If False, the data is padded on the fly in a pattern-by-pattern fashion, only original unpadded data is the RAM
    ...
    data_simulation_exitwave = params.get('data_simulation_exitwave', False)
    use_padded_intensity = params.get('use_padded_intensity', True)
    ...
    edge_slice_thickness=params.get('edge_slice_thickness', slice_distances[0]) ## The first and last slices of the object will be numerically thickened.
    edge_slice_thickness_multiplier = edge_slice_thickness / slice_distances[0]
    ...
    static_background=params.get('static_background', static_background)
    ...
    probe_mod_reg_weight = params.get('probe_mod_reg_weight', 0)
    probe_mod_tol = params.get('probe_mod_tol', 0.05)
    ...
    beta_wedge = params.get('beta_wedge', 0)
    ...
    # Constraints that modify the variables 'by hand'
    ...
    puzzle_positions=params.get('puzzle_positions', None)  ## added for puzzleing
    
    ### beam initialisation
    mc_n_modes=params.get('mc_n_modes', None) ## number of monte carlo probe modes
    focal_spread=params.get('focal_spread', 0)
    spatial_coh_x_A=params.get('spatial_coh_x_A', None) ## source size (x)
    spatial_coh_y_A=params.get('spatial_coh_y_A', None) # source size (y)
    if (spatial_coh_x_A is None) and not(spatial_coh_y_A is None):
        spatial_coh_x_A=1*spatial_coh_y_A
    if not(spatial_coh_x_A is None) and (spatial_coh_y_A is None):
        spatial_coh_y_A=1*spatial_coh_x_A
    if (spatial_coh_y_A is None) and (spatial_coh_x_A is None):
        spatial_coh_y_A, spatial_coh_x_A = 0, 0
    spatial_coh_xy=params.get('spatial_coh_xy', 0) # source size (xy correaltion strictly between -1 and 1)
    n_mc_samples=params.get('n_mc_samples', 256)
    ...
    params['pypty_version']="2.2"
    ############################### done with params, starts other things ###################################
    
    ### get the data
    if not(masks is None):
        masks, *_=pyptyutils.preprocess_dataset(masks, False, "lsq_sqrt", recon_type, data_shift_vector, data_bin, 0, upsample_pattern, 1, np, True)
    if dataset is None:
        if data_path[-2:]=="h5":
            ...
            if flip_ky:
                dataset=dataset[:,::-1,:]
            dataset, data_shift_vector, data_bin, data_pad_dummy, data_multiplier = pyptyutils.preprocess_dataset(dataset, False, algorithm, recon_type, data_shift_vector, data_bin, 0, upsample_pattern, data_multiplier, np, False)
    else:
    ...
    static_background = pyptyutils.create_static_background_from_nothing(static_background, probe, damping_cutoff_multislice,data_pad,upsample_pattern,  default_float_cpu, recon_type, use_padded_intensity) ## initializing static background
    ...
    try:
        ...
        sys.stdout.flush()
        obj, probe, positions,positions_correction, tilts, tilts_correction, masks, defocus_array, slice_distances, aperture_mask, dataset, static_background, aberrations_array, beam_current=pyptyutils.try_to_gpu(obj, probe, positions,positions_correction, tilts, tilts_correction, masks, defocus_array, slice_distances, aperture_mask, dataset, load_one_by_one, static_background, aberrations_array, beam_current, default_float, default_complex, default_int, xp) ##Convert numpy arrays to cupy arrays
    probe=pyptyutils.apply_probe_modulation(probe, extra_probe_defocus, acc_voltage, pixel_size_x_A, pixel_size_y_A, aberrations, print_flag, beam_ctf, n_hermite_probe_modes, defocus_spread_modes, probe_marker, mc_n_modes, focal_spread, spatial_coh_x_A, spatial_coh_y_A, spatial_coh_xy, data_pad, recon_type, n_mc_samples, default_complex, default_float, xp) #Here we will apply aberrations to an existing beam and create multiple modes
    ...
    dataset, data_shift_vector, data_bin, data_pad_dummy, data_multiplier = pyptyutils.preprocess_dataset(dataset, load_one_by_one, algorithm, recon_type, data_shift_vector, data_bin, 0, upsample_pattern, data_multiplier, xp, force_pad)
    #######----------------------------------------------------------------------------------------------------------------------------------
    ...
    list_hyperparams=[propmethod, data_simulation_flag, data_simulation_exitwave, reset_positions, restart_from_vacuum, reset_history_flag, smart_memory, wolfe_c1_constant,wolfe_c2_constant, window_weight, hist_length, deformation_reg_weight_tilts, deformation_reg_weight_positions, slow_axis_reg_weight_positions, slow_axis_reg_weight_tilts, fast_axis_reg_weight_positions,fast_axis_reg_weight_tilts, update_step_bfgs, apply_gaussian_filter_amplitude, apply_gaussian_filter, keep_probe_states_orthogonal, loss_weight, phase_norm_weight, abs_norm_weight, probe_reg_constraint_weight, do_charge_flip, atv_weight, wedge_mu, beta_wedge, tune_only_probe_phase, mixed_variance_weight,mixed_variance_sigma, phase_only_obj, tune_only_probe_abs, dynamically_resize_yx_object, update_beam_current, update_probe, update_obj, update_probe_pos, update_tilts, update_static_background, update_aberrations_array, puzzle_positions, probe_mod_reg_weight, probe_mod_tol]
    keys_hyperparams=["propmethod", "data_simulation_flag", "data_simulation_exitwave", "reset_positions", "restart_from_vacuum", "reset_history_flag", "smart_memory", "wolfe_c1_constant","wolfe_c2_constant", "window_weight", "hist_length", "deformation_reg_weight_tilts", "deformation_reg_weight_positions", "slow_axis_reg_weight_positions", "slow_axis_reg_weight_tilts", "fast_axis_reg_weight_positions","fast_axis_reg_weight_tilts", "update_step_bfgs", "apply_gaussian_filter_amplitude", "apply_gaussian_filter", "keep_probe_states_orthogonal", "loss_weight", "phase_norm_weight", "abs_norm_weight", "probe_reg_constraint_weight", "do_charge_flip", "atv_weight", "wedge_mu", "beta_wedge", "tune_only_probe_phase", "mixed_variance_weight","mixed_variance_sigma", "phase_only_obj", "tune_only_probe_abs", "dynamically_resize_yx_object", "update_beam_current", "update_probe", "update_obj", "update_probe_pos", "update_tilts", "update_static_background", "update_aberrations_array", "puzzle_positions", "probe_mod_reg_weight", "probe_mod_tol"]
    counter_failed_epochs, epoch_fail_position_turnoff, epoch_fail_tilts_turnoff, epoch_fail_sb_turnoff, epoch_fail_aber_turnoff, epoch_fail_probe_turnoff = 0, np.inf, np.inf, np.inf, np.inf, np.inf ## variables for an autofixer
    for epoch in range(epoch_prev,epoch_max,1):
        ...
        current_propmethod, current_data_simulation_flag, current_data_simulation_exitwave, current_reset_positions, current_restart_from_vacuum, this_reset_history_flag, this_smart_memory, current_wolfe_c1_constant,current_wolfe_c2_constant, current_window_weight, current_hist_length, current_deformation_reg_weight_tilts, current_deformation_reg_weight_positions, current_slow_axis_reg_weight_positions, current_slow_axis_reg_weight_tilts, current_fast_axis_reg_weight_positions,current_fast_axis_reg_weight_tilts, current_update_step_bfgs, current_apply_gaussian_filter_amplitude, current_apply_gaussian_filter, current_keep_probe_states_orthogonal, current_loss_weight, current_phase_norm_weight, current_abs_norm_weight, current_probe_reg_constraint_weight, current_do_charge_flip, current_atv_weight, current_wedge_mu, current_beta_wedge, current_tune_only_probe_phase, current_mixed_variance_weight,current_mixed_variance_sigma, current_phase_only_obj, current_tune_only_probe_abs, current_dynamically_resize_yx_object, current_beam_current_step, current_probe_step, current_obj_step, current_probe_pos_step, current_tilts_step, current_static_background_step, current_aberrations_array_step, current_puzzle_positions, current_probe_mod_reg_weight, current_probe_mod_tol =               pyptyutils.get_value_for_epoch(list_hyperparams, epoch, default_float_cpu) ## here we get the values of constraints for this epoch
        
        ######### Autofixer for the stuck reconstruction #########
        if counter_failed_epochs>=2: ## if the update can not be done at least two times (first with history and then without history), we try to manually fix this issue by fixing certain variables and giving others more time to evolve into a better state.
            epoch_fail_position_turnoff=1*epoch
            warning="\nWARNING: Scan grid refinement was manually disabled for the next 10 epochs!"
            sys.stdout.write(warning)
            warnings+=warning
            if counter_failed_epochs>=3:
                epoch_fail_tilts_turnoff=1*epoch
                warning="\nWARNING: Tilt grid refinement was manually disabled for the next 10 epochs!"
                sys.stdout.write(warning)
                warnings+=warning
                if counter_failed_epochs>=4:
                    epoch_fail_sb_turnoff=1*epoch
                    warning="\nWARNING: Static background refinement was manually disabled for the next 10 epochs!"
                    sys.stdout.write(warning)
                    warnings+=warning
                if counter_failed_epochs>=5:
                    epoch_fail_aber_turnoff=1*epoch
                    warning="\nWARNING: Aberrations refinement was manually disabled for the next 10 epochs!"
                    sys.stdout.write(warning)
                    warnings+=warning
                    if counter_failed_epochs>=6:
                        epoch_fail_probe_turnoff=1*epoch
                        warning="\nWARNING: Probe refinement was manually disabled for the next 10 epochs!"
                        sys.stdout.write(warning)
                        warnings+=warning
                    if counter_failed_epochs>=7:
                        warning="\nWARNING: Reconstruction is stuck and no automatic tools were effective, contact Anton Gladyshev!"
                        sys.stdout.write(warning)
                        warnings+=warning
            sys.stdout.flush()
        if np.abs(epoch_fail_position_turnoff-epoch)<10:  ## turn off position refinement and wait for new object and probe
            current_probe_pos_step=0
        if np.abs(epoch_fail_tilts_turnoff-epoch)<10:  ## turn off tilt refinement and wait for new object and probe
            current_tilts_step=0
        if np.abs(epoch_fail_sb_turnoff-epoch)<10:  ## turn off sb refinement and wait for new object and probe
            current_static_background_step=0
        if np.abs(epoch_fail_aber_turnoff-epoch)<10:  ## turn off aberrations refinement and wait for new object and probe
            current_aberrations_array_step=0
        if np.abs(epoch_fail_probe_turnoff-epoch)<10:  ## turn off probe refinement and wait for new object and probe
            current_probe_step=0
        ######### End of  autofixer for the stuck reconstruction #########
        ...
        for st in [current_beam_current_step, current_probe_step, current_obj_step, current_probe_pos_step, current_tilts_step, current_static_background_step, current_aberrations_array_step]:
            new_steps_sum+=st!=0
        if epoch!=epoch_prev and (constratins_new!=constratins_prev or this_reset_history_flag or new_steps_sum!=prev_steps_sum):
            reset_bfgs_history()
            if print_flag:
        ...
        count, save_flag=0, False ## count for measurements, not really used in BFGS (currently default) but one can use it for stochastic descent
...
def bfgs_update(algorithm_type, this_slice_distances, edge_slice_thickness_multiplier, this_step_probe, this_step_obj, this_step_pos_correction, this_step_tilts, measured_array, this_wavelength, masks, pixel_size_x_A, pixel_size_y_A, phase_norm_weight, abs_norm_weight, stepsize_threshold_low, probe_reg_weight, aperture_mask, recon_type, defocus_array, Cs, alpha_near_field, damping_cutoff_multislice, smooth_rolloff, update_extra_cut, keep_probe_states_orthogonal, do_charge_flip, cf_delta_phase, cf_delta_abs, cf_beta_phase, cf_beta_abs, phase_only_obj, wedge_mu, beta_wedge, wolfe_c1_constant, wolfe_c2_constant, atv_weight, atv_q, atv_p, tune_only_probe_phase, propmethod, this_chopped_sequence, load_one_by_one, data_multiplier,data_pad, phase_plate_in_h5, print_flag, this_loss_weight, max_count, reduce_factor, optimism, mixed_variance_weight, mixed_variance_sigma, data_bin, data_shift_vector, smart_memory, default_float, default_complex, default_int, upsample_pattern, this_step_static_background, tilt_mode, fancy_sigma, tune_only_probe_abs, aberration_marker, this_step_aberrations_array, probe_marker, compute_batch, current_window, current_window_weight, dynamically_resize_yx_object, lazy_clean, current_gaussian_filter, current_apply_gaussian_filter_amplitude, this_beam_current_step, xp, remove_fft_cache, is_first_epoch, hist_length, actual_step, fast_axis_reg_weight_positions, fast_axis_reg_weight_tilts, slow_axis_reg_weight_positions, slow_axis_reg_weight_tilts,  scan_size, scan_step_A, current_deformation_reg_weight_positions, current_deformation_reg_weight_tilts, warnings, data_simulation_flag, data_simulation_exitwave, pos_puzzling, probe_mod_reg_weight,probe_mod_tol, use_padded_intensity, counter_failed_epochs):
    ...
    constraint_contributions : list of float
    ...
    if probe_reg_weight==xp.inf:
        ...
        if type(aperture_mask)==xp.ndarray:
            probe=pyptyutils.fourier_clean(probe, mask=aperture_mask, default_float=default_float, xp=xp)
        else:
            probe=pyptyutils.fourier_clean(probe, cutoff=aperture_mask, default_float=default_float, xp=xp)
    if update_probe:
        if is_mixed_state and keep_probe_states_orthogonal:
            probe=pyptyobjective.make_states_orthogonal(probe, xp=xp)
        probe = pyptyutils.fourier_clean(probe, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
    
    if update_obj:
        if do_charge_flip:
            obj=pyptyobjective.charge_flip(obj, cf_delta_phase, cf_delta_abs, cf_beta_phase, cf_beta_abs, fancy_sigma, xp=xp);
            reset_bfgs_history()
            empty_hist=True
        if not lazy_clean:
            if smart_memory:
                for i in range(obj.shape[2]):
                    for j in range(obj.shape[3]):
                        what=obj[:,:,i,j]
                        obj[:,:,i,j]=pyptyutils.fourier_clean(what, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
            else:
                obj=pyptyutils.fourier_clean(obj,   cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
    try:
        pool.free_all_blocks()
        pinned_pool.free_all_blocks()
        if smart_memory and remove_fft_cache:
            cp.fft.config.clear_plan_cache()
    except:
        pass
            
    if empty_hist:
        total_loss, this_sse, this_object_grad, this_probe_grad, this_pos_grad, this_tilts_grad, static_background_grad, this_grad_aberrations_array, this_beam_current_grad, constraint_contributions = pyptyobjective.loss_and_direction(obj, probe, positions, positions_correction, tilts, tilts_correction, this_slice_distances, edge_slice_thickness_multiplier, measured_array,  algorithm_type, this_wavelength, update_probe, update_obj, update_pos_correction, update_tilts, masks, pixel_size_x_A, pixel_size_y_A, recon_type, Cs, defocus_array, alpha_near_field, damping_cutoff_multislice, smooth_rolloff_loss, propmethod, this_chopped_sequence, load_one_by_one, data_multiplier, data_pad, phase_plate_in_h5, this_loss_weight, data_bin, data_shift_vector, upsample_pattern, static_background, update_static_background, tilt_mode, aberration_marker, probe_marker, aberrations_array, compute_batch, phase_only_obj, beam_current, update_beam_current, update_aberrations_array, default_float, default_complex, xp, is_first_epoch, scan_size, scan_step_A, fast_axis_reg_weight_positions, slow_axis_reg_weight_positions, slow_axis_reg_weight_tilts, current_deformation_reg_weight_positions, current_deformation_reg_weight_tilts, fast_axis_reg_weight_tilts, aperture_mask, probe_reg_weight, current_window_weight, current_window, phase_norm_weight, abs_norm_weight, atv_weight, atv_q, atv_p, mixed_variance_weight, mixed_variance_sigma, wedge_mu, beta_wedge, probe_mod_reg_weight,probe_mod_tol, smart_memory, print_flag, data_simulation_flag, data_simulation_exitwave, use_padded_intensity)
        if clean_grad_ap:
            if type(aperture_mask)==xp.ndarray:
                this_probe_grad=pyptyutils.fourier_clean(this_probe_grad, mask=aperture_mask, default_float=default_float, xp=xp)
            else:
                this_probe_grad=pyptyutils.fourier_clean(this_probe_grad, cutoff=aperture_mask, default_float=default_float, xp=xp)
        if update_probe:
            this_probe_grad = pyptyutils.fourier_clean(this_probe_grad, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
            ...
            if smart_memory:
                for i in range(obj.shape[2]):
                    for j in range(obj.shape[3]):
                        ...
                        this_object_grad[:,:,i,j]=pyptyutils.fourier_clean(what, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
            else:
                this_object_grad=pyptyutils.fourier_clean(this_object_grad,   cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
        if phase_only_obj:
            this_object_grad=pyptyutils.complex_grad_to_phase_grad(this_object_grad, obj, xp=xp)
        if tune_only_probe_abs:
            this_probe_grad_fourier=pyptyfft.ifft2(this_probe_grad, xp=xp)*this_probe_grad.shape[0]*this_probe_grad.shape[1]
            this_probe_fourier=pyptyfft.fft2(probe, xp=xp)
            probe_fourier_mag,probe_fourier_phase=xp.sqrt(xp.abs(this_probe_fourier)), xp.angle(this_probe_fourier)
            this_probe_grad=pyptyutils.complex_grad_to_mag_grad(this_probe_grad_fourier, probe_fourier_mag,probe_fourier_phase, xp=xp)
            tune_only_probe_phase=False
        if tune_only_probe_phase:
            this_probe_grad_fourier=pyptyfft.ifft2_ishift(this_probe_grad, xp=xp) * this_probe_grad.shape[0] * this_probe_grad.shape[1]
            this_probe_fourier=pyptyfft.shift_fft2(probe, xp=xp)
            probe_fourier_abs, probe_fourier_phase = xp.abs(this_probe_fourier), xp.angle(this_probe_fourier)
            this_probe_grad=pyptyutils.complex_grad_to_phase_grad(this_probe_grad_fourier, this_probe_fourier, xp=xp)
        this_obj_update=-1*this_object_grad if update_obj else 0
        d_val_obj=(2-phase_only_obj)*xp.sum(xp.abs(this_obj_update)**2)
        if d_val_obj==0:
            d_val_obj=1
        this_probe_update= -1*this_probe_grad*d_val_obj/((2-tune_only_probe_abs-tune_only_probe_phase)*xp.sum(xp.abs(this_probe_grad)**2)) if update_probe else 0
        this_pos_update= -1*this_pos_grad*d_val_obj/(xp.sum(this_pos_grad**2)) if update_pos_correction else 0
        this_tilts_update= -1*this_tilts_grad*d_val_obj/(xp.sum(this_tilts_grad**2)) if update_tilts else 0
        this_static_background_update= -1*static_background_grad*d_val_obj/(xp.sum(static_background_grad**2)) if update_static_background else 0
        this_aberrations_array_update =-1*this_grad_aberrations_array*d_val_obj/(xp.sum(this_grad_aberrations_array**2)) if update_aberrations_array else 0
        this_beam_current_update=-1*this_beam_current_grad*d_val_obj/(xp.sum(this_beam_current_grad**2)) if update_beam_current else 0
        ...
        for subdumb_bfgs_i in range(len(rhos_hist)-1,-1,-1):
            ...
            siTq=0
            if update_obj:
                siTq+=(2-phase_only_obj)*xp.sum(xp.real(history_bfgs["obj_hist_s"][subdumb_bfgs_i]*xp.conjugate(this_obj_update)))
            if update_probe:
                siTq+=(2-tune_only_probe_abs-tune_only_probe_phase)*xp.sum(xp.real(history_bfgs["probe_hist_s"][subdumb_bfgs_i]*xp.conjugate(this_probe_update)))
            if update_pos_correction:
                siTq+=xp.sum(history_bfgs["positions_hist_s"][subdumb_bfgs_i]*this_pos_update)
            if update_tilts:
                siTq+=xp.sum(history_bfgs["tilts_hist_s"][subdumb_bfgs_i]*this_tilts_update)
            if update_static_background:
                siTq+=xp.sum(history_bfgs["static_background_hist_s"][subdumb_bfgs_i]*this_static_background_update)
            if update_aberrations_array:
                siTq+=xp.sum(history_bfgs["aberrations_hist_s"][subdumb_bfgs_i]*this_aberrations_array_update)
            if  update_beam_current:
                siTq+=xp.sum(history_bfgs["beam_current_hist_s"][subdumb_bfgs_i]*this_beam_current_update)
            alphai=rhoi*siTq
        ...
        for subdumb_bfgs_i in range(len(rhos_hist)):
            ...
            yiTr=0
            if update_obj: yiTr+=(2-phase_only_obj)*xp.sum(xp.real(history_bfgs["obj_hist_y"][subdumb_bfgs_i]*xp.conjugate(this_obj_update)));
            if update_probe: yiTr+=(2-tune_only_probe_abs-tune_only_probe_phase)*xp.sum(xp.real(history_bfgs["probe_hist_y"][subdumb_bfgs_i]*xp.conjugate(this_probe_update)));
            if update_pos_correction: yiTr+=xp.sum(history_bfgs["positions_hist_y"][subdumb_bfgs_i]*this_pos_update);
            if update_tilts: yiTr+=xp.sum(history_bfgs["tilts_hist_y"][subdumb_bfgs_i]*this_tilts_update);
            if update_static_background: yiTr+=xp.sum(history_bfgs["static_background_hist_y"][subdumb_bfgs_i]*this_static_background_update);
            if update_aberrations_array: yiTr+=xp.sum(history_bfgs["aberrations_hist_y"][subdumb_bfgs_i]*this_aberrations_array_update);
            if update_beam_current: yiTr+=xp.sum(history_bfgs["beam_current_hist_y"][subdumb_bfgs_i]*this_beam_current_update);
            ...
            if smart_memory:
                for i in range(obj.shape[2]):
                    for j in range(obj.shape[3]):
                        ...
                        this_obj_update[:,:,i,j]=pyptyutils.fourier_clean(what, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
            else:
                this_obj_update=pyptyutils.fourier_clean(this_obj_update,   cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
        #### END of BFGS update construction
    d_value=0
    if update_obj: d_value+=(2-phase_only_obj)*xp.sum(xp.real(xp.conjugate(this_obj_update)*this_object_grad));
    if update_probe: d_value+=(2-tune_only_probe_phase)*xp.sum(xp.real(xp.conjugate(this_probe_update)*this_probe_grad));
    if update_pos_correction: d_value+=xp.sum(this_pos_update*this_pos_grad);
    if update_tilts: d_value+=xp.sum(this_tilts_update*this_tilts_grad);
    if update_static_background: d_value+=xp.sum(this_static_background_update*static_background_grad);
    if update_aberrations_array: d_value+=xp.sum(this_aberrations_array_update*this_grad_aberrations_array);
    if update_beam_current: d_value+=xp.sum(this_beam_current_update*this_beam_current_grad);
        
    if d_value>=0: ### This should not happen if wolfe 2 is satisfied, but just in case the loss is not a smooth curve we do:
        sys.stdout.write("\n\n\nWarning! Positive dir-derivative, using normal gradient-descent!\n\n\n\n\n\n")
    ...
    while True:
            ...
            if not lazy_clean:
                if smart_memory:
                    for i in range(obj.shape[2]):
                        for j in range(obj.shape[3]):
                            what=new_obj[:,:,i,j]
                            new_obj[:,:,i,j]=pyptyutils.fourier_clean(what, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
                else:
                    new_obj=pyptyutils.fourier_clean(new_obj,   cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
        else:
            new_obj=obj
            
        if update_probe:
            if tune_only_probe_phase:
                probe_fourier=pyptyfft.shift_fft2(probe, xp=xp)
                probe_fourier_abs, probe_fourier_angle=xp.abs(probe_fourier), xp.angle(probe_fourier)
                new_probe=pyptyfft.ifft2_ishift(probe_fourier_abs * xp.exp(1j*(probe_fourier_angle + actual_step*this_probe_update)), xp=xp)
            else:
                if tune_only_probe_abs:
                    probe_fourier=pyptyfft.shift_fft2(probe, xp=xp)
                    probe_fourier_abs, probe_fourier_angle = xp.abs(probe_fourier), xp.angle(probe_fourier)
                    new_probe=pyptyfft.ifft2_ishift((probe_fourier_abs+ actual_step*this_probe_update) * xp.exp(1j*(probe_fourier_angle)), xp=xp)
                else:
        ...
        new_total_loss, new_sse, new_object_grad, new_probe_grad, new_pos_grad, new_tilts_grad, new_static_background_grad, new_grad_aberrations_array, new_beam_current_grad, new_constraint_contributions = pyptyobjective.loss_and_direction(new_obj, new_probe, positions, new_positions_correction, tilts, new_tilts_correction, this_slice_distances, edge_slice_thickness_multiplier, measured_array,  algorithm_type, this_wavelength, update_probe, update_obj, update_pos_correction, update_tilts, masks, pixel_size_x_A, pixel_size_y_A, recon_type, Cs, defocus_array, alpha_near_field, damping_cutoff_multislice, smooth_rolloff_loss, propmethod, this_chopped_sequence, load_one_by_one, data_multiplier, data_pad, phase_plate_in_h5, this_loss_weight, data_bin, data_shift_vector, upsample_pattern, new_static_background, update_static_background, tilt_mode, aberration_marker, probe_marker, new_aberrations_array, compute_batch, phase_only_obj, new_beam_current, update_beam_current, update_aberrations_array, default_float, default_complex, xp, is_first_epoch, scan_size, scan_step_A, fast_axis_reg_weight_positions,slow_axis_reg_weight_positions, slow_axis_reg_weight_tilts, current_deformation_reg_weight_positions, current_deformation_reg_weight_tilts, fast_axis_reg_weight_tilts, aperture_mask, probe_reg_weight, current_window_weight, current_window, phase_norm_weight, abs_norm_weight, atv_weight, atv_q, atv_p, mixed_variance_weight, mixed_variance_sigma, wedge_mu, beta_wedge, probe_mod_reg_weight,probe_mod_tol, smart_memory, print_flag, data_simulation_flag, data_simulation_exitwave, use_padded_intensity)
        
        if clean_grad_ap:
            if type(aperture_mask)==xp.ndarray:
                new_probe_grad=pyptyutils.fourier_clean(new_probe_grad, mask=aperture_mask, default_float=default_float, xp=xp)
            else:
                new_probe_grad=pyptyutils.fourier_clean(new_probe_grad, cutoff=aperture_mask, default_float=default_float, xp=xp)
        if update_probe:
            new_probe_grad = pyptyutils.fourier_clean(new_probe_grad, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
        if smart_memory:
            try:
                if remove_fft_cache:
                    cp.fft.config.clear_plan_cache()
            ...
            if smart_memory:
                for i in range(new_obj.shape[2]):
                    for j in range(new_obj.shape[3]):
                        ...
                        new_object_grad[:,:,i,j]=pyptyutils.fourier_clean(what, cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
            else:
                new_object_grad=pyptyutils.fourier_clean(new_object_grad,   cutoff=damping_cutoff_multislice-update_extra_cut, rolloff=smooth_rolloff, default_float=default_float, xp=xp)
        if phase_only_obj:
            new_object_grad=pyptyutils.complex_grad_to_phase_grad(new_object_grad, new_obj, xp=xp)
        if tune_only_probe_abs:
            this_probe_grad_fourier=pyptyfft.ifft2(new_probe_grad, xp=xp)*new_probe_grad.shape[0]*new_probe_grad.shape[1]
            new_probe_fourier=pyptyfft.fft2(new_probe, xp=xp)
            probe_fourier_mag,probe_fourier_phase=xp.sqrt(xp.abs(new_probe_fourier)), xp.angle(new_probe_fourier)
            new_probe_grad=pyptyutils.complex_grad_to_mag_grad(new_probe_fourier, probe_fourier_mag,probe_fourier_phase, xp=xp)
        if tune_only_probe_phase:
            new_probe_grad_fourier=pyptyfft.ifft2_ishift(new_probe_grad, xp=xp) * new_probe_grad.shape[0] * new_probe_grad.shape[1]
            new_probe_fourier=pyptyfft.shift_fft2(new_probe, xp=xp)
            probe_fourier_abs, probe_fourier_phase = xp.abs(new_probe_fourier), xp.angle(new_probe_fourier)
            new_probe_grad=pyptyutils.complex_grad_to_phase_grad(new_probe_grad_fourier, new_probe_fourier, xp=xp)
        new_d_value=0
        if update_obj: new_d_value+=(2-phase_only_obj)*xp.sum(xp.real(xp.conjugate(this_obj_update)*new_object_grad));
        if update_probe: new_d_value+=(2-tune_only_probe_phase)*xp.sum(xp.real(xp.conjugate(this_probe_update)*new_probe_grad));
        if update_pos_correction: new_d_value+=xp.sum(this_pos_update*new_pos_grad);
        if update_tilts: new_d_value+=xp.sum(this_tilts_update*new_tilts_grad);
        if update_static_background: new_d_value+=xp.sum(this_static_background_update*new_static_background_grad);
        if update_aberrations_array: new_d_value+=xp.sum(this_aberrations_array_update*new_grad_aberrations_array);
        if update_beam_current: new_d_value+=xp.sum(this_beam_current_update*new_beam_current_grad);
        ...
        if this_wolfe_1 and this_wolfe_2:
            ...
            if not(this_wolfe_2 or this_wolfe_1):
                actual_step=type(actual_step)( 10**(np.random.uniform(0,20)-20) ) ## pick a random step and try again
            ...
            if actual_step<stepsize_threshold_low:
                break
        ##############################################################################################################################
    if not(this_wolfe_1): ## checking Wolfe_1 again
        ...
        counter_failed_epochs+=1
        sys.stdout.flush()
        reset_bfgs_history()
        return total_loss, this_sse, constraint_contributions, actual_step, count, d_value, new_d_value, warnings, counter_failed_epochs
   
    counter_failed_epochs=0
    if print_flag>=2:
    ...
    yiTsi_tot=0
    if update_obj:
        yiTyi=xp.sum(xp.abs(history_bfgs["obj_hist_y"][-1])**2);
        yiTsi=(2-phase_only_obj)*xp.sum(xp.real(history_bfgs["obj_hist_y"][-1]*xp.conjugate(history_bfgs["obj_hist_s"][-1])));
        gamma=yiTsi/yiTyi
        yiTsi_tot+=yiTsi
        history_bfgs["gamma_obj"]=gamma
    if update_probe:
        yiTyi=xp.sum(xp.abs(history_bfgs["probe_hist_y"][-1])**2);
        yiTsi=(2-tune_only_probe_abs-tune_only_probe_phase)*xp.sum(xp.real(history_bfgs["probe_hist_y"][-1]*xp.conjugate(history_bfgs["probe_hist_s"][-1])));
        yiTsi_tot+=yiTsi
        gamma=yiTsi/yiTyi
        history_bfgs["gamma_probe"]=gamma
    if update_pos_correction:
        yiTyi=xp.sum((history_bfgs["positions_hist_y"][-1])**2);
        yiTsi=xp.sum(history_bfgs["positions_hist_y"][-1]*history_bfgs["positions_hist_s"][-1]);
        yiTsi_tot+=yiTsi
        gamma=yiTsi/yiTyi
        history_bfgs["gamma_pos"]=gamma
    if update_tilts:
        yiTyi=xp.sum((history_bfgs["tilts_hist_y"][-1])**2);
        yiTsi=xp.sum(history_bfgs["tilts_hist_y"][-1]*history_bfgs["tilts_hist_s"][-1]);
        yiTsi_tot+=yiTsi
        gamma=yiTsi/yiTyi
        history_bfgs["gamma_tilts"]=gamma
    if update_static_background:
        yiTyi=xp.sum((history_bfgs["static_background_hist_y"][-1])**2);
        yiTsi=xp.sum(history_bfgs["static_background_hist_y"][-1]*history_bfgs["static_background_hist_s"][-1]);
        yiTsi_tot+=yiTsi
        gamma=yiTsi/yiTyi
        history_bfgs["gamma_static"]=gamma
    if update_aberrations_array:
        yiTyi=xp.sum((history_bfgs["aberrations_hist_y"][-1])**2);
        yiTsi=xp.sum(history_bfgs["aberrations_hist_y"][-1]*history_bfgs["aberrations_hist_s"][-1]);
        yiTsi_tot+=yiTsi
        gamma=yiTsi/yiTyi
        history_bfgs["gamma_aberrations"]=gamma
    if update_beam_current:
        yiTyi=xp.sum((history_bfgs["beam_current_hist_y"][-1])**2);
        yiTsi=xp.sum(history_bfgs["beam_current_hist_y"][-1]*history_bfgs["beam_current_hist_s"][-1]);
        yiTsi_tot+=yiTsi
        gamma=yiTsi/yiTyi
            ...
            for i in range(obj.shape[2]):
                for j in range(obj.shape[3]):
                    ...
                    if current_gaussian_filter:
                        ...
                        if current_gaussian_filter:
                            what_angle=xp.ndi.gaussian_filter(what_angle, current_gaussian_filter, order=0, mode='nearest')
                    ...
                    if current_apply_gaussian_filter_amplitude:
                        ...
                        what_abs=xp.ndi.gaussian_filter(what_abs, current_apply_gaussian_filter_amplitude, order=0, mode='nearest')
    ...
    if update_pos_correction and dynamically_resize_yx_object>0:
        if xp.max(xp.abs(new_positions_correction))>=dynamically_resize_yx_object:
            if print_flag:
                sys.stdout.write("\nRemoving an integer overflow in sub-pixel position correction!")
                sys.stdout.flush()
    ...
    if pos_puzzling and update_pos_correction: ### check if grid is split into multiple chunks and correct it if it is.
        ...
        puzzling_worked,positions_puzzled =pyptyutils.position_puzzling(full_positions,scan_size,sigma=0.4,score_threshold=1.2)
        if puzzling_worked:
            obj_dtype=obj.dtype
            obj = pyptyutils.intorplate_puzzled_obj(obj,full_positions,positions_puzzled,order=1,method = 'cubic', xp=xp)
            obj = xp.asarray(obj).astype(obj_dtype)
            positions_dtype=positions.dtype
            positions_correction_dtype=positions_correction.dtype
            
            positions_correction=(positions_puzzled-np.round(positions_puzzled))
            positions=xp.round(xp.asarray(positions_puzzled)).astype(positions_dtype)
            positions_correction=xp.asarray(positions_correction).astype(positions_correction_dtype)
            reset_bfgs_history()
