...
try:
    ...
    print("Warning: skimage not found!")
from collections import defaultdict
from scipy.ndimage import map_coordinates
from pypty import se as pyptyse
import math
...
def fourier_clean_3d(array, cutoff=0.66, mask=None, rolloff=0, default_float=cp.float32, xp=cp):
    ...
    if not(cutoff is None) or not(mask is None):
        shape=array.shape
        if mask is None:
            x=pyptyfft.fftshift(pyptyfft.fftfreq(shape[2], xp=xp),  xp=xp)
            y=pyptyfft.fftshift(pyptyfft.fftfreq(shape[1], xp=xp),  xp=xp)
            ...
            del x,y, r
        arrayff=pyptyfft.shift_fft2(array, axes=(1,2), overwrite_x=True, xp=xp)
        if len(shape)==6:
        ...
        arrayff=pyptyfft.ifft2_ishift(arrayff, axes=(1,2), overwrite_x=True, xp=xp)
        del mask
        return arrayff
    else:
        return array
...
def fourier_clean(array, cutoff=0.66, mask=None, rolloff=0, default_float=cp.float32, xp=cp):
    ...
    if not(cutoff is None) or not(mask is None):
        shape=array.shape
        if mask is None:
            x=pyptyfft.fftshift(pyptyfft.fftfreq(shape[1], xp=xp), xp=xp)
            y=pyptyfft.fftshift(pyptyfft.fftfreq(shape[0], xp=xp), xp=xp)
            ...
            if rolloff!=0:
                ...
                mask[r>r0]*=0.5*(1+cp.cos(3.141592654*(r-r0)/(max_r**2-r0)))[r>r0]
            del x,y, r
        arrayff=pyptyfft.shift_fft2(array, axes=(0,1), xp=xp)
        if len(shape)==4:
            arrayff=arrayff*mask[:,:, None, None]
        if len(shape)==3:
            arrayff=arrayff*mask[:,:, None]
        ...
        arrayff=pyptyfft.ifft2_ishift(arrayff, axes=(0,1), xp=xp)
        del mask
        return arrayff
    else:
        return array
...
def create_spatial_frequencies(px, py, shape, damping_cutoff_multislice, smooth_rolloff, default_float, xp=cp):
    """
    ...
    """
    qx,qy= xp.meshgrid(pyptyfft.fftfreq(shape, px, xp=xp), pyptyfft.fftfreq(shape, py, xp=xp), indexing="xy")
    ...
    exclude_mask=pyptyfft.fftshift(exclude_mask_ishift, xp=xp)
...
def generate_mask_for_grad_from_pos(shapex, shapey, positions_list, shape_footprint_x,shape_footprint_y, shrink=0):
    ...
    return mask
def complex_grad_to_phase_grad(grad, array, xp=cp):
    """
    ...
    """
    return 2*xp.real(-1j*xp.conjugate(array)*grad)
def complex_grad_to_phase_abs_grad(grad, array, xp=cp):
    """
    ...
    """
    array_real, array_imag, grad_real, grad_imag = xp.real(array), xp.imag(array), xp.real(grad), xp.imag(grad)
    return 2*(grad_imag*array_real - array_imag*grad_real), -2*(array_real*grad_real + array_imag*grad_imag)
    
def complex_grad_to_mag_grad(grad, abs, phase, xp=cp):
    """
    ...
    """
    array = abs * xp.exp(-1j * phase)
    return 4 * xp.real(grad * array)
...
def get_window(shape, r0, r_max, inverted=True, xp=cp):
    """
    ...
    mask=xp.ones_like(r)
    if r0<r_max:
        mask[xp.abs(x)>r0]*=0.5*(1+xp.cos(3.141592654*(xp.abs(x)-r0)/(r_max-r0)))[xp.abs(x)>r0]
        mask[xp.abs(y)>r0]*=0.5*(1+xp.cos(3.141592654*(xp.abs(y)-r0)/(r_max-r0)))[xp.abs(y)>r0]
    mask[xp.abs(x)>r_max]=0
    mask[xp.abs(y)>r_max]=0
    if inverted:
        mask=1-mask
    return mask
...
def create_static_background_from_nothing(static_background, probe, damping_cutoff_multislice, data_pad, upsample_pattern, default_float, recon_type, use_padded_intensity):
    """
    ...
    if not(static_background is None):
        if type(static_background)!=np.ndarray:
            if static_background<1e-7: static_background=1e-7;
...
def apply_probe_modulation(probe, extra_probe_defocus, acc_voltage, pixel_size_x_A, pixel_size_y_A, aberrations, print_flag, beam_ctf, n_hermite_probe_modes, defocus_spread_modes, probe_marker, mc_n_modes, focal_spread, spatial_coh_x_A, spatial_coh_y_A, spatial_coh_xy, data_pad,recon_type, n_mc_samples,  default_complex, default_float, xp):
    """
    ...
    if not(aberrations is None):
        ...
        if len(probe.shape)==3:
            probe=pyptyfft.ifft2_ishift(pyptyfft.shift_fft2(probe, xp=xp)*xp.exp(-1j*ctf)[:,:,None], xp=xp)
        else:
            probe=pyptyfft.ifft2_ishift(pyptyfft.shift_fft2(probe, xp=xp)*xp.exp(-1j*ctf)[:,:,None, None], xp=xp)
    if not(beam_ctf is None):
        beam_ctf= xp.asarray(beam_ctf)
        if beam_ctf.shape[0]!=probe.shape[0]:
            beam_ctf=xp.pad(beam_ctf, data_pad)
        if len(probe.shape)==3:
            probe=pyptyfft.ifft2_ishift(pyptyfft.shift_fft2(probe, xp=xp)*xp.exp(-1j*beam_ctf)[:,:,None], xp=xp)
        else:
            probe=pyptyfft.ifft2_ishift(pyptyfft.shift_fft2(probe, xp=xp)*xp.exp(-1j*beam_ctf)[:,:,None,None], xp=xp)
        if print_flag:
            sys.stdout.write("\nUsing the provided CTF for the beam initial guess!\n")
    if not(mc_n_modes is None):
        probe=probe_modes_from_spreads(
            probe[:,:,:1],
            wavelength,
            wavelength/(probe.shape[1]*pixel_size_x_A),
            focal_spread,
            spatial_coh_x_A,
            spatial_coh_y_A,
            spatial_coh_xy,
        ...
        if not(defocus_spread_modes is None):
            if probe_marker is None:
                p_final=xp.zeros((probe.shape[0], probe.shape[1], len(defocus_spread_modes)), dtype=default_complex)
                ...
                probe2=xp.zeros((probe.shape[0], probe.shape[1], len(defocus_spread_modes), probe.shape[3]), dtype=default_complex)
...
def resample_slices(o, params, refactor_thickness=1, N_before=0, N_after=0):
    """
    ...
    refactor_thickness : float, optional
        Factor by which to refine slice sampling (default is 1). Values > 1 increase
    ...
    osy,osx, osz, osm=o.shape
    slice_distances=params["slice_distances"]
    ms=np.mean(slice_distances)
    if N_before>0:
        o=np.concatenate((np.ones((osy,osx, N_before, osm)), o), axis=2)
        if len(slice_distances)>1: slice_distances=np.concatenate((np.ones(N_before)*ms, slice_distances), axis=0);
    if N_after>0:
        o=np.concatenate((o, np.ones((osy,osx, N_after, osm))), axis=2)
        if len(slice_distances)>1: slice_distances=np.concatenate((slice_distances, np.ones(N_before)*ms), axis=0);
    
    if refactor_thickness!=1:
        if len(slice_distances)!=1:
            raise ValueError('Slice sampling must be homogenious for thickness refactoring!')
        z_old=np.arange(osz)*slice_distances[0]
        osz_new=int(np.ceil(osz*refactor_thickness))
        slice_distances = slice_distances /refactor_thickness
        z_new=np.arange(osz_new)*slice_distances[0]
        o= np.moveaxis(o, 2, 0)
        interp_func = interp1d(z_old, o, axis=0, kind='linear', fill_value="extrapolate")
        o= interp_func(z_new)
        
        o=o**(1/refactor_thickness)
        o= np.moveaxis(o, 0, 2)

    params["slice_distances"]=slice_distances
    params["total_thickness"]=slice_distances*o.shape[2]
    params["obj"]=o
    return o, params
...
def probe_modes_from_spreads(
    probe,
    wavelength,
    rez_px_size_rad,       # radians per pixel
    focal_spread,               # focal spread std (Å)
    spatial_coh_x_A=0,
    spatial_coh_y_A=0,
    spatial_coh_xy=0,
    n_modes=4,
    ...
    seed=0,
    data_pad=0,
    recon_type="far_field",
    xp=np
):
    """
    Generate a coherent-mode expansion of a partially coherent probe via MC sampling + SVD.

    The probe is perturbed by random transverse shifts (spatial coherence) and
    ...
    vectors are returned as coherent modes in real space.

    Parameters
    ----------
    probe : numpy.ndarray or cupy.ndarray
        Probe array. Expected shape is ``(Ny, Nx, 1)`` (real-space). Only the first
        mode is used as the starting point.
    wavelength : float
        Electron wavelength in Å.
    rez_px_size_rad : float
        Reciprocal-space sampling in radians per pixel.
    focal_spread : float
        Standard deviation of defocus spread (Å).
    spatial_coh_x_A : float, optional
        RMS spatial coherence in x (Å). Default is 0.
    spatial_coh_y_A : float, optional
        RMS spatial coherence in y (Å). Default is 0.
    spatial_coh_xy : float, optional
    ...
    data_pad : int, optional
        Padding (pixels) to crop before SVD and then restore afterwards for far-field
        reconstructions (default is 0).
    recon_type : {"far_field", "near_field"}, optional
        If not ``"near_field"``, the function optionally crops/pads the probe in
        reciprocal space by `data_pad` (default is "far_field").
    xp : module, optional
        Array module, typically ``numpy`` or ``cupy``.

    Returns
    -------
    modes_r : numpy.ndarray or cupy.ndarray
    ...
    sys.stdout.write(
    "\n*****************************************************************************\n****************** Running Monte-Carlo probe initialization *****************\n*****************************************************************************")

    A0_k=pyptyfft.shift_fft2(pyptyfft.fftshift(probe[:,:,0], xp=xp), xp=xp)
    
    if data_pad!=0 and recon_type!="near_field":
        A0_k=A0_k[data_pad:-data_pad, data_pad:-data_pad]
    
    A0_k = xp.asarray(A0_k)
    Ny, Nx = A0_k.shape

    kx_pix, ky_pix = xp.meshgrid(
        xp.arange(Nx) - Nx // 2,
        xp.arange(Ny) - Ny // 2,
        indexing="xy"
    )
    kx_rad = kx_pix * rez_px_size_rad
    ky_rad = ky_pix * rez_px_size_rad


    Sigma_pos_A2=np.array([[spatial_coh_x_A**2,  spatial_coh_x_A*spatial_coh_y_A*spatial_coh_xy],
                           [spatial_coh_x_A*spatial_coh_y_A*spatial_coh_xy, spatial_coh_y_A**2]])
                           
    dx, dy = np.random.multivariate_normal(mean=[0, 0], cov=Sigma_pos_A2, size=n_samples).T
    df = np.random.normal(size=n_samples, scale=focal_spread)

    dx, dy, df = xp.asarray(dx), xp.asarray(dy), xp.asarray(df)
    
    
    chi_shift = (2.0 * xp.pi / wavelength) * (
        kx_rad[None, :, :] * dx[:, None, None] +
        ky_rad[None, :, :] * dy[:, None, None]
    )

    k2 = (kx_rad**2 + ky_rad**2)[None, :, :]
    chi_def = (xp.pi / wavelength) * (df[:, None, None] * k2)
    
    Psi = A0_k[None, :, :] * xp.exp(-1j * (chi_shift + chi_def))

    X = Psi.reshape(n_samples, Ny * Nx).T  # (Npix, Ns)

    U, S, _ = xp.linalg.svd(X, full_matrices=False)

    lambdas = (S ** 2) / float(n_samples)  # (Ns,)

    r = min(int(n_modes), U.shape[1])
    total_power = float(xp.sum(lambdas))
    captured_power = float(xp.sum(lambdas[:r]))
    frac_captured = captured_power / (total_power + 1e-30)


    sys.stdout.write("\nModes used: %d"%n_modes)
    sys.stdout.write("\nCaptured power: %.4f"%frac_captured)
    sys.stdout.flush()
    
    modes_k = U[:, :r].reshape(Ny, Nx, r)

    weights = lambdas[:r].copy()  # (r,)
    modes_k = modes_k * xp.sqrt(weights)[None, None, :]

    order = xp.argsort(weights)[::-1]
    modes_k = modes_k[:, :, order]
    weights = weights[order]
    
    if data_pad!=0 and recon_type!="near_field":
        try:
            modes_k=xp.pad(modes_k, [[data_pad,data_pad],[data_pad,data_pad],[0,0]])
        except:
            modes_k=xp.pad(modes_k, [[data_pad,data_pad],[data_pad,data_pad],[0,0],[0,0]])
    
    
    modes_r = pyptyfft.fftshift(
        pyptyfft.ifft2_ishift(modes_k, xp=xp),
        xp=xp
    )
...
def apply_defocus_probe(probe, distance, acc_voltage, pixel_size_x_A, pixel_size_y_A, default_complex, default_float, xp):
    ...
    q2, qx, qy, exclude_mask, exclude_mask_ishift=create_spatial_frequencies(pixel_size_x_A, pixel_size_y_A,probe.shape[0], 2/3, 0, default_float, xp=xp)
    q2, exclude_mask_ishift, =xp.asarray(q2), xp.asarray(exclude_mask_ishift)
    propagator_phase_space=xp.exp(-1j*xp.pi*distance*wavelength*q2)*exclude_mask_ishift
    if len(probe.shape)==3:
        wave_fourier=pyptyfft.fft2(probe, axes=(0,1), xp=xp)*propagator_phase_space[:,:,None]
    else:
        wave_fourier=pyptyfft.fft2(probe, axes=(0,1), xp=xp)*propagator_phase_space[:,:,None, None]
    return pyptyfft.ifft2(wave_fourier, axes=(0,1), xp=xp)
        ...
        with open(output_folder+"loss.csv", mode='a', newline='') as loss_list:
            ...
            if save_loss_log==2:
                write_loss.writerow({"epoch": epoch,
                                ...
                                "Probe-Modulation": constraint_contributions[13],
...
def load_nexus_params(path_nexus):
    ...
    """
    pypty_params = {}
    with h5py.File(path_nexus, "r") as f:
        path_inside = "entry/reconstruction/reconstruction parameters"
        for k in f[path_inside].keys():
            if k == "dataset":
                continue

            dset = f[path_inside + "/" + k]
            value = dset[()]

            # Handle string datasets written as 1-element arrays
            ...
            # Undo "auto" placeholder
            if value == "auto":
                value = None

            # Reinterpret complex from string (optional)
            if isinstance(value, str):
                try:
                    # Try interpreting stringified complex like "(1+2j)"
                    value_eval = complex(value)
                    value = value_eval
                except:
...
def convert_to_nxs(folder_path, output_file):
    ...
    edge_slice_thickness_multiplier=metadata.get("edge_slice_thickness_multiplier", 1)
    ...
    with h5py.File(output_file, "w") as f:
        ...
        obj_grp.create_dataset("z", data=np.arange(1-edge_slice_thickness_multiplier, co.shape[1]+1-edge_slice_thickness_multiplier, 1)  * slice_spacing)
        ...
        # Scan positions
        scan_grp = instr_grp.create_group("scan")
        ...
        recon_grp.create_dataset("version", data=["v2.2"])
        recon_grp.create_dataset("date", data=[creation_time])
        recon_grp.create_dataset("folder", data=[metadata.get("output_folder", "")])
        try:
            recon_grp.create_dataset("dataset", data=[metadata.get("data_path", "/auto").split("/")[-1]])
        except:
            pass
        ...
        for key, value in metadata.items():
            if key in {"probe", "obj", "mean_pattern", "masks"}:
                continue  # these are stored elsewhere or not needed

            try:
                # Sanitize bad scalars
                if type(value)!=np.ndarray:
                    if (
                        ...
                        # Scalars like 12.5 or 100 are OK as-is
                        pass
                        ...
                        if value.dtype == object or np.issubdtype(value.dtype, np.complexfloating):
                            continue  # skip problematic arrays
...
def phase_cross_corr_align(im_ref_fft, im_2_fft, refine_box_dim, upsample, x_real, y_real, shift_y=None, shift_x=None):
    ...
    return im_2_fft_shifted, shift_y, shift_x
...
def segment_regions_from_image(image: np.ndarray,sigma: float = 1.0):
    """
    ...
    closed = binary_closing(edge_mask,disk(1))
...
def interpolate_label0_points(points: np.ndarray, method='linear', fallback='nearest'):
    ...
    fallback : {"nearest", None}, optional
        Backup method used where the primary interpolation returns NaNs (default is
        "nearest").
...
def optimize_2d_block_shifts(points: np.ndarray):
    ...
    - The discrete Laplacian of the resulting 2D displacement field is minimized
      (smoothness / low curvature), and
    ...
    result = minimize(objective, shift_init)
    # print(result)
...
def position_puzzling(points,scan_size,sigma=0.4,score_threshold=1.2):
    ...
    try:
        ...
        points_np=1*points
    y = points_np[:,0].reshape(scan_size)
    x = points_np[:,1].reshape(scan_size)
    ...
    if score>score_threshold :
        ...
        labeled_regions, num_regions, edge_mask = segment_regions_from_image(mask, sigma=sigma)
        if num_regions < 2:
            sys.stdout.write("\nWarning! %d segmentations found,no need to puzzle,score is %.2f"%(num_regions, score))
        else:
            ...
            sys.stdout.write("\nWarning! %d segmentations found,start to puzzle,score is %.2f"%(num_regions, score))
    else:
        sys.stdout.write("\nNo segmentation found, using original points,score is %.2f"%score)
...
def intorplate_puzzled_obj(array,pos_old,pos_new,order=1,method = 'linear', xp=cp):
    """
